...
//   response.end("Раз раз");
// })
// server.listen(3000, function() {console.log('ggg');
// });
...
for (let i = 0; i < phrases.length; i++) {
  let elem = [];
  phrasesResult.push(elem);
  
  for (let j = 0; j < phrases[i].length; j++) {
    if (j == 0) {
      for (let k = 0; k < phrases[i][0].length; k++) {
        let obj = {id: '', phraseRu: '', phraseEn: ''};
        phrasesResult[i].push(obj);
      }
    }
  }
}

for (let i = 0; i < phrasesResult.length; i++){
  for (let j = 0; j < phrasesResult[i].length; j++) {
    phrasesResult[i][j].id = j + 1;
    phrasesResult[i][j].phraseRu = phrases[i][0][j];
    phrasesResult[i][j].phraseEn = phrases[i][1][j];
  }
}
...
fs.writeFile("correctlyFile.js", JSON.stringify(phrasesResult), () => {
